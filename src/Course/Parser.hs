...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P pa1) (P pa2) = P $ \input -> case pa1 input of
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  (=<<) apb (P pa) = P $ \input -> onResult (pa input) (\i a -> parse (apb a) i)
...
instance Applicative Parser where
  ...
  -- (<*>) (P pab) (P pa) = P $ \input -> onResult (pab input) (\i ab -> onResult (pa i) (\i' a -> Result i' (ab a)))
...
(.:.) ::
  ...
  -> Parser (List a)
(.:.) pa pla = lift2 (:.) pa pla
-- (.:.) pa pla = P $ \i -> case parse pa i of
                           -- Result i' a -> case parse pla i' of
                                            -- Result i'' al -> Result i'' (a :. al)
                                                        -- -> Result i' (a :. Nil)
                           -- _           -> Result i Nil
...
-- sequenceParser (p :. ps) = lift2 (:.) p (sequenceParser ps)
...
personParser = Person <$> ageParser <* spaces1 <*> firstNameParser <* spaces1
                      <*> surnameParser <* spaces1 <*> smokerParser
                      <* spaces1 <*> phoneParser
