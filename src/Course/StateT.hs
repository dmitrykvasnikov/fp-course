...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<$>) ab ska = StateT ((<$>) (first ab) . runStateT ska)
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<*>) sab sa = StateT $ \s -> runStateT sab s >>= \(ab, s') -> runStateT sa s' >>= \(a, s'') -> pure (ab a, s'')
...
instance Monad k => Monad (StateT s k) where
  (=<<) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (=<<) askb ska = StateT $ \s -> runStateT ska s >>= \(a,s') -> runStateT (askb a) s'
...
distinct' ::
  ...
  List a
  -> List a
distinct' la = eval' (filtering (\a -> state' (\s -> (S.notMember a s, S.insert a s))) la) S.empty
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF la = evalT (filtering (\a -> StateT $ \s -> if a > 100 then Empty else (pure (S.notMember a s, S.insert a s))) la) S.empty
...
instance Functor k => Functor (OptionalT k) where
  (<$>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<$>) ab ao = OptionalT . ((ab <$>) <$>) . runOptionalT $ ao
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (<*>) (OptionalT oab) (OptionalT oa) = OptionalT $ oab >>= optional (\ab -> ((ab <$>) <$> oa)) (pure Empty)
...
instance Monad k => Monad (OptionalT k) where
  (=<<) ::
    ...
    -> OptionalT k a
    -> OptionalT k b
  (=<<) aob oa = OptionalT $ go =<< runOptionalT oa
                 where go Empty = pure Empty
                       go (Full a) = runOptionalT $ aob a
